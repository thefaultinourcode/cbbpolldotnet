...
export const getUserpollTest = async (poll) => {
	await connectMongo();

	const ballots = await UserBallot.find({ _id: { $in: poll.ballots}});
	console.log('get userpoll ballots:', ballots);
...
export const getPoll = async (week) => {
	...
	let poll = await Poll.findOne({ week: week });
	poll = JSON.parse(JSON.stringify(poll));
	return poll;
};

export const checkPoll = async (week, season) => {
	let poll = await Poll.findOne({week: week, season: season});
	console.log('checkPoll:', poll);
	let pollExists;
	if(poll == null){
		console.log('Poll is null');
		pollExists = false;
	}
	else{
		pollExists = true;
...
export const addPoll = async (pollData) => {
    ...
    const response = await Poll.create(pollData);
