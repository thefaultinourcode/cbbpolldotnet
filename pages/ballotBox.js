...
export default function BallotBox(props) {
	...
	const {
		...
		return fetch("/api/isPollOpen").then((res) => res.json());
	});
	console.log("isPollOpen", isPollOpen);
	...
	if (props.ballot) {
		...
		textarea = (
			<textarea
				...
				cols="150"
				id="overallReasoning"
				className="overallRationale"
				maxLength={2000}
				defaultValue={props.ballot.overallReasoning}
			></textarea>
		);
	} else {
		console.log("false");
		ballotComp = (
			<Ballot teams={props.teams} update={updateBallot} reasoning={updateReasoning}></Ballot>
		);
		textarea = (
			<textarea
				rows="5"
