...
export default function UserProfile(props) {
	...
	for (let i = 0; i < ballotLength2023; i++) {
		let link = (
			<tr key={ballots2023[i]._id} className="ballotCell">
				<td>
					<Link href={`/ballots/${ballots2023[i].week}/${ballots2023[i]._id}`}>
						<a>Week {ballots2023[i].week}</a>
					</Link>
				</td>
			</tr>
		);
	...
	for (let i = 0; i < ballots2024.length; i++) {
		let link = (
			<tr key={ballots2024[i]._id} className="ballotCell">
				<td>
					<Link href={`/ballots/${ballots2024[i].week}/${ballots2024[i]._id}`}>
						<a>Week {ballots2024[i].week}</a>
					</Link>
				</td>
			</tr>
		);
	...
	if (secondaryTeam && tertiaryTeam) {
		alsoSupports = (
			<h2>
				Also supports: <Image src={secondaryTeam.url} width={30} height={30}></Image> <Image src={tertiaryTeam.url} width={30} height={30}></Image>
			</h2>
		);
	} else if (secondaryTeam) {
		alsoSupports = (
			<h2>
				Also supports: <Image src={secondaryTeam.url} width={30} height={30}></Image>{' '}
			</h2>
		);
	} else {
		alsoSupports = '';
	...
	return (
		<div>
			{navbar}
			<h1>
				{verified} <Image src={primaryTeam.url} width={40} height={40}></Image> {profile.userprofile}
			</h1>
			{alsoSupports}
